const colors = [
	"#76946A", "#C34043", "#DCA561", "#E82424",
	"#FF9E3B", "#6A9589", "#658594", "#727169",
	"#938AA9", "#957FB8", "#7E9CD8", "#9CABCA",
	"#7FB4CA", "#A3D4D5", "#7AA89F", "#98BB6C",
	"#938056", "#C0A36E", "#E6C384", "#D27E99",
	"#E46876", "#FF5D62", "#FFA066", "#717C7C",
];
...
const isColorUniqueInSquare =  color => colorsUsedInSameSquare.indexOf(color) === -1;

const isColorDifferentFromAdjacentSquares = (color, innerSquareType, rowPosition, columnPosition) => {
...
const getInnerSquareBackgroundColor = (innerSquareType, index) => {
	...
	const isColorValid = isColorUniqueInSquare(color) && isColorDifferentFromAdjacentSquares(color, innerSquareType, rowPosition, columnPosition);
...
const createAndAppendInnerSquareElement = (container, index) => innerSquareType => {
	const element = document.createElement('div');

	element.classList.add(innerSquareType);
	element.style.backgroundColor = getInnerSquareBackgroundColor(innerSquareType, index);
...
const getSquaresPerSideFromUrl = (urlSearchParams) => {
	...
	const isSquaresPerSideValid = Number.isInteger(Number(squaresPerSide)) && squaresPerSide > 0;
...
const setFrameParameters = () => {
	...
	frameParameters.outerSquareDimension = Math.floor(frameDimension * 10 / squaresPerSide) / 10;
	frameParameters.innerSquareDimension = 0.5 * Math.sqrt(2 * Math.pow(frameParameters.outerSquareDimension, 2));
};
...
const generateFrame = () => {
	...
	for (let index = 0; index < Math.pow(frameParameters.squaresPerSide, 2); index += 1) {
